class Day5
  ...
  def self.part1
    program = File.read('day05-input.txt').chomp.split(",").map(&:to_i)
    Intcode.run(input)
  end
...
class Intcode
  def self.run(program, debug: false)
    @debug = debug
    memory = program.dup
...
describe Intcode do
  context 'Day5 programs' do
    it 'example one' do
      ...
      expect(Intcode.run(program)).to eq([42, 0, 4, 0, 99])
      expect{Intcode.run(program)}.to output("input: \n42\n").to_stdout
    end

    it 'example two' do
      program = [1002,4,3,4,33]
      expect(Intcode.run(program)).to eq([1002,4,3,4,99])
    end
  ...
  context 'can still run Day2 programs' do
    [ { program: [1,0,0,0,99], end_state: [2,0,0,0,99] },
      ...
      it example[:program].to_s do
        expect(Intcode.run(example[:program])).to eq(example[:end_state])
      end
    end
