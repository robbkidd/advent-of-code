...
describe Grid do
  ...
  describe 'finding neighbors' do
    describe 'in cardinal directions' do
      let(:grid) { Grid.new(example_input_2022_day_12).parse.set_cardinal_neighbors }
    ...
    describe 'in surrounding directions' do
      let(:grid) { Grid.new(example_input_2022_day_12).parse.set_surrounding_neighbors }
    ...
    it 'raises exception when grid neighbor type is not set yet' do
      grid = Grid.new(example_input_2022_day_12)
      expect {
        grid.neighbors_for([0,0])
      }.to raise_error("You must set_cardinal_neighbors or set_surrounding_neighbors for this Grid.")
    end

    it 'raises exception when grid not parsed yet' do
      grid = Grid.new(example_input_2022_day_12).set_cardinal_neighbors
      expect {
        grid.neighbors_for([0,0])
